...
trait JsonDecoders {
  ...
  implicit val CountryCodeReader: Decoder[CountryCode] =
    Decoder[String].map(a => CountryCode.withName(a))
  implicit val CurrencyReader: Decoder[Currency] =
  ...
  implicit val PreCheckoutQueryReader: Decoder[PreCheckoutQuery] =
    deriveDecoder[PreCheckoutQuery]
  implicit val ShippingQueryReader: Decoder[ShippingQuery] =
